...
class AudioFixer:
    def __init__(self, output_dir_path, noise_file_path, mic_amp=9, debug=False):
    ...
    def fix_audio(self, input_video_path):
        ...
        if not mic_path.is_file():
            print(f'{input_video_path.name}: Video has only 1 audio track.\n')
            return
...
if __name__ == '__main__':
    video_path = Path('D:/Videos/')  # Can be a single file or folder (don't forget '.mp4' suffix).
